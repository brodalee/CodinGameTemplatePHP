...
class WatcherCompiler implements IFileSystemListener {
    ...
    {
        // Ignoring all files that are not in src directory.
        if (!str_starts_with(
                str_replace('\\', '/', $event->getPath()),
                str_replace('\\','/',__DIR__ . '/src/')
            )
        ) {
            return false;
        }
...
$listener = new WatcherCompiler;

$fs = new FireFS();
$fs->setListener($listener);
$fs->setWorkingDir("./src");
